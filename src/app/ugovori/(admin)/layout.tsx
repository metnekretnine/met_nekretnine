...
export default async function EPotpisLayout({ children }: { children: React.ReactNode }) {
  ...
  const content = <EPotpisAdmin cmsData={cms.app} authenticated={access === "allowed"} authMode={authMode}>{children}</EPotpisAdmin>;
  if (test || !clerkConfigured()) return content;
  const { sessionId } = await auth();
  return <ClerkProvider localization={hrHR} signInUrl="/ugovori" signInFallbackRedirectUrl="/ugovori" afterSignOutUrl="/ugovori"
