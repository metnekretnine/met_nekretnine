...
import { shortContractPath, type ContractPaths } from "@/lib/epotpis/routes";
...
interface AdminState {
  paths: ContractPaths; cms: EPotpisTexts; contracts: AdminContract[]; hasSignature: boolean; signatureUrl: string; busy: boolean;
...
interface Props { paths: ContractPaths; cmsData: EPotpisTexts; authenticated: boolean; authMode: "clerk" | "test" | "unconfigured"; children: ReactNode }
...
export function EPotpisAdmin({ cmsData: c, paths, authenticated, authMode, children }: Props) {
  const observedPath = usePathname(), router = useRouter();
  const pathname = paths.home === "/" ? shortContractPath(observedPath) : observedPath;
  ...
  async function logout() {
    ...
    await run(async () => {
      ...
      router.replace(paths.home); router.refresh();
    });
    setLoggingOut(false);
  }
  ...
  function showCreated(link: string) { setFeedback({ path: paths.home, error: "", notice: c.createdNotice, link }); }
  const state: AdminState = { paths, cms: c, contracts, hasSignature, signatureUrl, busy, createdLink: current.link, refresh, run, setError, setNotice, setCreatedLink, copy, showCreated };
  const title = pathname === paths.create ? c.newContract : pathname === paths.settings ? c.settings : pathname === paths.profile ? c.myProfile : c.title;
  return <AdminContext.Provider value={state}><div className="ep-root">
    {loggedIn && <header className="ep-header"><Link href={paths.home} className="ep-brand"><EPotpisLogo aria-label={c.brand} /></Link>
      <nav><Link href={paths.settings} title={c.settings} aria-label={c.settings} aria-current={pathname === paths.settings ? "page" : undefined}
        aria-disabled={busy} onClick={event => { if (busy) event.preventDefault(); }}><Settings2 size={19} /><span>{c.settings}</span></Link>
        {authMode !== "test" && <Link href={paths.profile} title={c.myProfile} aria-label={c.myProfile}><UserRound size={19} /><span>{c.myProfile}</span></Link>}
        {authMode === "test" ? <button title={c.logout} aria-label={c.logout} disabled={busy} onClick={() => void logout()}>{loggingOut ? <span className="ep-loader-spinner" /> : <LogOut size={19} />}</button>
          : <SignOutButton redirectUrl={paths.home}><button title={c.logout} aria-label={c.logout} disabled={busy}><LogOut size={19} /></button></SignOutButton>}</nav>
    </header>}
    <main className={`ep-main ${!loggedIn ? "ep-login-main" : ""}`}>
      {!loggedIn ? authMode === "unconfigured" ? <section className="ep-panel ep-login"><EPotpisLogo className="ep-login-logo" aria-label={c.brand} /><h1>{c.loginTitle}</h1><p role="alert">{c.authNotConfigured}</p></section>
        ...
        {pathname !== paths.home && <Link className="ep-back" href={paths.home} aria-disabled={busy} onClick={event => { if (busy) event.preventDefault(); }}><ArrowLeft size={17} />{c.back}</Link>}
        <div className="ep-heading"><h1 id="ep-heading" tabIndex={-1}>{title}</h1>
          {pathname === paths.home && <Link className="ep-button" href={paths.create} aria-disabled={busy || !dataReady}
        ...
        {dataReady || pathname === paths.profile ? children : !current.error ? <EPotpisLoader label={c.loading} /> : null}
      </>}
    </main>
