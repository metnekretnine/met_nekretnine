...
export function EPotpisContracts() {
  ...
  const filtered = contracts.filter(item => (filter === "all" || item.status === filter) && `${item.ownerName} ${item.propertyAddress}`.toLocaleLowerCase().includes(search.toLocaleLowerCase()));
  ...
  return <>
        ...
        <section className="ep-panel ep-list"><div className="ep-list-toolbar"><div className="ep-tabs">{["all", "sent", "signed"].map(item => <button key={item} className={filter === item ? "is-active" : ""} onClick={() => setFilter(item)}>{c[item as keyof typeof c]}</button>)}</div>
          ...
          {!filtered.length ? <div className="ep-empty"><FileText size={32} /><h2>{contracts.length ? c.noResults : c.emptyTitle}</h2><p>{c.emptyDescription}</p></div> : <div className="ep-table-wrap"><table><thead><tr>{["owner", "property", "status", "actions"].map(key => <th key={key} className={key === "property" ? "ep-contract-property" : undefined}>{c[key as keyof typeof c]}</th>)}</tr></thead><tbody>
            {filtered.map(item => <tr key={item.id}><td><strong>{item.ownerName}</strong><small>{new Date(item.createdAt).toLocaleDateString("hr-HR")}</small></td><td className="ep-contract-property">{item.propertyAddress}<small>{item.kind === "open" ? c.open : c.exclusive}</small></td>
              ...
              <td><div className="ep-row-actions">{item.status !== "failed" && <a href={`/api/ugovori/contracts/${item.id}/pdf?download=1`} title={c.download} aria-label={c.download}><Download size={17} /></a>}
                {(item.status === "sent" || item.status === "signed") && <><button title={c.copyLink} aria-label={c.copyLink} onClick={() => void copy(item.signUrl)}><Copy size={17} /></button><a href={item.status === "signed" ? `/api/ugovori/contracts/${item.id}/pdf` : item.signUrl} target="_blank" rel="noreferrer" title={c.viewDocument} aria-label={c.viewDocument}><ExternalLink size={17} /></a></>}
